...
function setIdentity(identifier, data, callback)
  ...
  MySQL.Async.execute(
    'INSERT INTO characters (identifier, firstname, lastname, dateofbirth, sex) VALUES (@identifier, @firstname, @lastname, @dateofbirth, @sex)',
    {
      ['@identifier'] = identifier,
      ['@firstname']  = data.firstname,
      ['@lastname']   = data.lastname,
...
function deleteIdentity(identifier, data, callback)
  MySQL.Async.execute("DELETE FROM `characters` WHERE identifier = @identifier AND firstname = @firstname AND lastname = @lastname AND dateofbirth = @dateofbirth AND sex = @sex",
    {
      ['@identifier']   = identifier,
      ['@firstname']    = data.firstname,
      ['@lastname']     = data.lastname,
...
TriggerEvent('es:addGroupCommand', 'idhelp', "user", function(source, args, user)
  ...
  TriggerClientEvent('chatMessage', source, "IDHelp", {255, 0, 0}, "Nicht genug Rechte!")
...
TriggerEvent('es:addGroupCommand', 'char', "user", function(source, args, user)
  ...
  TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Nicht genug Rechte!")
...
TriggerEvent('es:addGroupCommand', 'charlist', "user", function(source, args, user)
  getCharacters(source, function(data)
    if data.firstname1 ~= '' then
      ...
      TriggerClientEvent('chatMessage', source, 'CHARLIST', {255, 0, 0}, "Du hast keine Zeichen. Bitte benutze den /register command.")
    end
  end)
end, function(source, args, user)
  TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Nicht genug Rechte!")
  ...
  getCharacters(source, function(data)
      ...
      if data.firstname ~= '' then
        updateIdentity(GetPlayerIdentifiers(source)[1], data, function(callback)
          if callback == true then
            ...
            TriggerClientEvent('chatMessage', source, "CHARSELECT", {255, 0, 0}, "Identität aktualisieren fehlgeschlagen!")
          end
        end)
      else
        TriggerClientEvent('chatMessage', source, "CHARSELECT", {255, 0, 0}, "Du hast keinen Charakter in Slot 1!")
      end
    elseif charNumber == 2 then
      ...
      if data.firstname ~= '' then
        ...
        TriggerClientEvent('chatMessage', source, "CHARSELECT", {255, 0, 0}, "Du hast keinen Charakter in Slot 2!")
      end
    elseif charNumber == 3 then
      ...
      if data.firstname ~= '' then
        ...
        TriggerClientEvent('chatMessage', source, "CHARSELECT", {255, 0, 0}, "Du hast keinen Charakter in Slot 3!")
      end
    else
      TriggerClientEvent('chatMessage', source, "CHARSELECT", {255, 0, 0}, "Identität aktualisieren fehlgeschlagen!")
    end
      ...
      if data.firstname ~= '' then
        deleteIdentity(GetPlayerIdentifiers(source)[1], data, function(callback)
          ...
          TriggerClientEvent('chatMessage', source, "DELCHAR", {255, 0, 0}, "You Have Deleted " .. data.firstname .. " " .. data.lastname .. "!")
            ...
            TriggerClientEvent('chatMessage', source, "DELCHAR", {255, 0, 0}, "Identität löschen fehlgeschlagen!")
          end
        end)
      else
        TriggerClientEvent('chatMessage', source, "DELCHAR", {255, 0, 0}, "Du hast keinen Charakter in Slot 1!")
      end
    elseif charNumber == 2 then
      local data = {
        identifier   = data.identifier,
      ...
      if data.firstname ~= '' then
        ...
        TriggerClientEvent('chatMessage', source, "DELCHAR", {255, 0, 0}, "Du hast keinen Charakter in Slot 2!")
      end
    elseif charNumber == 3 then
      local data = {
        identifier   = data.identifier,
      ...
      if data.firstname ~= '' then
        ...
        TriggerClientEvent('chatMessage', source, "DELCHAR", {255, 0, 0}, "Du hast keinen Charakter in Slot 3!")
      end
    else
      TriggerClientEvent('chatMessage', source, "DELCHAR", {255, 0, 0}, "Identität löschen fehlgeschlagen!")
    end
